...
// Ubah endpoint /start jadi POST dan ambil videoUrl dari body
app.post('/start', async (req, res) => {
  ...
  try {
    ...
    if (!videoUrl) {
      return res.status(400).send('Link YouTube belum diisi.');
    }

    const videoId = getVideoIdFromUrl(videoUrl);
    if (!videoId) {
      return res.status(400).send('Link video YouTube tidak valid.');
    }
