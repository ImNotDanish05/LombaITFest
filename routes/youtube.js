...
const router = express.Router(); // BUKAN app
const fs = require('fs');
...
const oauth2Client = new google.auth.OAuth2(
  YTdata.client_id,
  YTdata.client_secret,
  YTdata.redirect_uris[0]
);
...
// Hapus komentar spam
router.post('/delete-spam', express.urlencoded({ extended: true }), async (req, res) => {
  if (!fs.existsSync(TOKEN_PATH)) {
    return res.send('Belum login. <a href="/login">Login dulu</a>');
  }
  try {
...
router.get('/blockedwords', (req, res) => {
...
router.post('/blockedwords', (req, res) => {
